...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      setError(error === 'Invalid login credentials' ? t('邮箱或密码错误', 'Invalid email or password') : error)
      setLoading(false)
      return
    }
    navigate('/dashboard')
        ...
        <form onSubmit={handleSubmit} className="bg-slate-900/50 border border-white/5 rounded-2xl p-8 space-y-5">
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs">
              <span className="px-3 bg-slate-900/50 text-slate-500">{t('或', 'Or continue with')}</span>
            </div>
          </div>
